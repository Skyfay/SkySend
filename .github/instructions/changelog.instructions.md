...
2. **One line per entry** - Each entry is a single bullet point. One sentence is ideal, two is the hard maximum. If you need a second sentence, ask yourself whether it is truly necessary.
3. **No implementation details** - No file paths, function names, internals, root causes, or technical explanations. Those belong in git commits.
...
9. **What, not why or how** - Describe what changed, not why it was done or how it was implemented. One sentence stating the user-visible change is ideal. Root causes, design rationale, and implementation details belong in git commits or pull request descriptions, not here.
