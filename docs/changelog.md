...
- **web**: Fixed Firefox freezing the entire browser for seconds to minutes during large-file downloads via Service Worker. With `highWaterMark: 0`, Firefox's download pipeline issued a synchronous IPC read on its main thread and blocked until the Service Worker's `pull()` enqueued a record. Any delay in `pull()` - network stall, TCP slow start, initial TLS handshake, or GC pause - held the main thread, making all browser tabs and UI unresponsive. The stream is now created with `highWaterMark: 2`, so `pull()` runs proactively and keeps two pre-decrypted records ready in the internal queue. Firefox reads from that queue without waiting, eliminating the IPC block. `pull()` still enqueues exactly one record per call, so Firefox's backpressure state remains correct and the stall-at-random-percentage regression from v2.9.2 cannot recur.
