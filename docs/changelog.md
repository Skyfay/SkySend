...
- **server**: Expanded test coverage across multiple server modules - `auth/index.ts` (was 0%, now 100%), `routes/download.ts` (100%), `routes/config.ts` (100%), `lib/config.ts` (100% lines). New tests cover `createOidcAdapter` provider resolution and fallback, timer-based cleanup intervals in `password-lockout.ts` and `rate-limit.ts`, OIDC discovery failure 503 responses, null token claims 400 response, OIDC protect flags in the config endpoint, invalid salt encoding in note creation, view-limit race condition in note viewing, middleware 400/404/401 edge cases, storage-missing 500 and download-race 410 in the download route, and all OIDC cross-field validation branches in `lib/config.ts`. Overall server line coverage improved from 84.44% to 90%.
