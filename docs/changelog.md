...
- **web**: Added a 30-second stall watchdog to the Service Worker download fallback path. When Firefox DevTools is opened during a download, the browser attaches async-task-tracking to the SW context. This overhead persists even after DevTools is closed and has no recovery path short of a full browser restart. Without the watchdog, the SW event loop becomes too saturated to produce decrypted records, Firefox buffers the entire encrypted file (up to 2 GB) in its receive buffer, and the download hangs indefinitely. The watchdog detects no-progress for 30 s, calls `reader.cancel()` to release the buffered memory, and sends `dl-error` so the frontend can show the user an actionable error message.
...
- **web**: The stall watchdog is now armed only after the first plaintext record is successfully produced. Previously it started immediately, which could trigger a false-positive "stalled" error on legitimate slow-network connections where no data arrived in the first 30 seconds. The watchdog now only fires if decryption was actively making progress and then stopped.
- **web**: When the stall watchdog fires, the download now aborts with a user-visible error message instead of silently falling back to the in-memory Blob tier. The error message tells the user that Firefox DevTools was likely open during the download and instructs them to close DevTools, restart Firefox, and try again. This prevents the previous silent behavior where a large file would start loading entirely into RAM via Blob, potentially exhausting available memory.
- **web**: Added a proactive Firefox DevTools warning, shown before a download starts when docked DevTools is detected and the file is larger than 256 MB. The warning explains why DevTools causes the download pipeline to stall and offers "Download anyway" for users who want to proceed. This uses a window dimension heuristic (outer vs. inner dimensions) to detect docked panels - undocked DevTools cannot be detected this way, but the stall watchdog and error message handle that case as a fallback.
...
- **web**: Added an in-app debug panel to the Download and Upload pages. The panel shows which download tier was used (Service Worker / Save File Picker / Blob), which SW decrypt path ran (Worker thread or ReadableStream fallback), the upload transport (WebSocket or HTTP chunks), browser name and version, and whether Firefox DevTools was detected. The panel is collapsed by default and expands on click, so users can diagnose transport issues without opening browser DevTools (which in Firefox can itself cause the download stall being diagnosed).
