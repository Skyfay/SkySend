...
export type UploadWorkerMessage =
  ...
  | { type: "transport"; transport: "ws" | "http"; wsFailed: boolean }
  | {
      type: "done";
      id: string;
      ownerToken: string;
...
self.onmessage = async (e: MessageEvent<UploadWorkerRequest>) => {
  ...
  try {
    ...
    if (wsUsable && wsInstance) {
      ...
      post({ type: "transport", transport: "ws", wsFailed: false });
      ...
      post({ type: "transport", transport: "http", wsFailed: wsEnabled && !wsUsable });
