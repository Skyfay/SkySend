...
export interface UploadCredentials {
  ...
  passwordAlgo?: "argon2id" | "argon2id-v2";
}
...
export async function prepareUpload(password?: string): Promise<UploadCredentials> {
  ...
  let passwordAlgo: "argon2id" | "argon2id-v2" | undefined;
...
export async function prepareDownload(
  ...
  if (password && passwordSaltB64 && passwordAlgo) {
    ...
    if (passwordAlgo === "argon2id") {
      // Legacy uploads (pre-v2.5.0) - use old Argon2id params for backward compat
      ...
      // Current uploads (v2.5.0+) - use current Argon2id params
      passwordKey = await deriveKeyFromPasswordArgon2(
        password,
        passwordSalt,
        hashWasmArgon2,
